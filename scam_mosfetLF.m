...
for k1=1:legMos
    ...
    %Gm next:
    MosFile{1}(k1+j)={['Gm',rename]};
    MosFile{2}(k1+j) = fileIn{2}(MosInd(k1));   % Get the two node numbers
    MosFile{3}(k1+j) = fileIn{3}(MosInd(k1));
    MosFile{4}(k1+j) = fileIn{4}(MosInd(k1));
    MosFile{5}(k1+j) = fileIn{3}(MosInd(k1));
    MosFile{6}(k1+j) = {''};
    %R0 next:
    MosFile{1}(k1+1+j)={['Ro',rename]};
    MosFile{2}(k1+1+j) = fileIn{2}(MosInd(k1));   % Get the two node numbers
    MosFile{3}(k1+1+j) = fileIn{3}(MosInd(k1));	
    MosFile{4}(k1+1+j) = {''};
    MosFile{5}(k1+1+j) = {''};
    MosFile{6}(k1+1+j) = {''};
end
