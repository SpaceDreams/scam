...
% The Mosfet is made up of G and R so nLines increase by one
oldnLines=nLines;
mLines=sum(ismember(cellfun (@(x) x(1),Name,'un',0),'M'));
%Adjust sizes to accomadate for added resistor and VCCS
Name{end+mLines}=Name{end};
N1(end+mLines)=N1(end);
N2(end+mLines)=N2(end);
arg3{end+mLines}=arg3{end};
arg4{end+mLines}=arg4{end};
arg5{end+mLines}=arg5{end};
...
for k1=1:nLines
    ...
    %M takes D S G B
    if Name{k1}(1)=='M'
        %Here we change the k1th row to a resistor 
        %then add a row and change the (kth+1) row 
        %to G, a VCCS.
        %Ro is from Drain to Source
        Name{k1}=['Ro',Name{k1}(2:end)];
        Name{oldnLines+mi}=['Gm',Name{k1}(2:end)];
        N1(oldnLines+mi)=N1(k1); N2(oldnLines+mi)=N2(k1);
        arg3{oldnLines+mi}=arg3{k1}; arg4{oldnLines+mi}=num2str(N2(k1));
        arg5{oldnLines+mi}=arg5{k1};
        mi=mi-1;	
    end
    switch Name{k1}(1)
        % Passive element
